            ...
            */
            ...
            void pass_two()
            {
                foreach (Instruction row in InstructionList)
                {
                    ...
                    /*else if(row.Symbol.Contains(DeviceAddress))
                    {
                        int val = Int32.Parse(row.Operand);
                        if(val > 64 || val < 0)
                        {
                            output += String.Format("{0} {1} {2}\nLine {3}: Device {4} Does Not Exist", row.Symbol, row.OpCode, row.Operand, row.LineNumber, row.Operand);
                            MessageBox.Show(output);
                            _process = PROCESS.ERROR;
                            return;
                        }
